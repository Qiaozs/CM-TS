...
"""
1. convDiscrimination: 卷积层（卷积+BN+relu+dropout）*3+池化层+全连接层，通道默认512-->2
2. Discrimination: 全连接层（全连接+BN+relu+dropout）*2+全连接，通道默认2048-->2
3. MAM: Modality Attention Module模块
"""
...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1):
    ...
    '''
    创建一个3×3卷积层，不指定输入输出通道数，步长stride大小为1，padding=dilation=1（dilation为卷积核内空洞，为1时代表没有空洞）
    bias=False不使用偏置。
    '''
...
class BasicBlock(nn.Module):
    # BasicBlock为ResNet 的基础模块，用于实现残差学习。
    expansion = 1
...
class Bottleneck(nn.Module):
    # Bottleneck为ResNet 的基础模块，用于实现残差学习。
    expansion = 4
