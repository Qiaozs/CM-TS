...
class ResNet_IBN(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x)  # [120, 3, 256, 128] -> [120, 64, 128, 64]
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)  # -> [120, 64, 64, 32]
        x = self.layer1(x)  # [120, 64, 64, 32] -> [120, 256, 64, 32]
        x = self.layer2(x)  # [120, 256, 64, 32] -> [120, 512, 32, 16]
        x = self.layer3(x)  # [120, 512, 32, 16] -> [120, 1024, 16, 8]
        x = self.layer4(x)  # [120, 1024, 16, 8] -> [120, 2048, 8, 4]
        x = self.avgpool(x)  # [120, 2048, 8, 4] -> [120, 2048, 1, 1]
        x = x.view(x.size(0), -1)
        x = self.fc(x)  # [120, 2048] -> [120, 1000]
        return x
